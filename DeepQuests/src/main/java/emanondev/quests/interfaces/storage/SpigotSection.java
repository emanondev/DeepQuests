...
import emanondev.quests.interfaces.storage.json.JsonSection;
import emanondev.quests.interfaces.storage.yaml.YmlSection;
...
public class SpigotSection implements ISpigotSection {
    ...
    public ItemStack getGuiItem(String path, ItemStack def) {
    	ItemStack result = getGuiItem(path);
        if (result == null) {
            update(path, def, false);
        }
        return result == null ? def : result;
    ...
    private void update(String path, Object value, boolean overwrite) {
        if (parent instanceof YmlSection) {
            ((YmlSection) parent).update(path, value, overwrite);
        } else {

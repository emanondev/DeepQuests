...
public class RewardManager {
	...
	private final static HashMap<String,RewardType> missionRewardsType =
			new HashMap<String,RewardType>();
	private final static HashMap<String,RewardType> questRewardsType =
			new HashMap<String,RewardType>();
	 ...
	 * 			additional info
	 */
	public LinkedHashMap<String,Reward> loadRewards(Mission m,MemorySection section){
			...
			if (keys!=null)
				keys.forEach((id)->{
					try {
						...
						Reward rew = missionRewardsType.get(key.toUpperCase()).getInstance((MemorySection) section.get(id), m);
						if (rew!=null)
	...
	public LinkedHashMap<String,Reward> loadRewards(Quest q,MemorySection section){
		LinkedHashMap<String,Reward> rewards = new LinkedHashMap<String,Reward>();
		if (section!=null) {
			Set<String> keys = section.getKeys(false);
			if (keys!=null)
				keys.forEach((id)->{
					try {
						...
						if (key== null || !questRewardsType.containsKey(key.toUpperCase()))
							throw new NullPointerException();
						Reward rew = questRewardsType.get(key.toUpperCase()).getInstance((MemorySection) section.get(id), q);
						if (rew!=null)
							rewards.put(rew.getNameID(),rew);
					} catch (Exception e) {
				...
				keys.forEach((id)->{
					try {
						...
						if (key== null || !rewardsType.containsKey(key.toUpperCase()))
							throw new NullPointerException();
						Reward rew = rewardsType.get(key.toUpperCase()).getInstance((MemorySection) section.get(id), gui);
						if (rew!=null)
							rewards.put(rew.getNameID(),rew);
					} catch (Exception e) {

...
public abstract class YmlLoadable implements Savable, WithGui {
	...
	public YmlLoadable(ConfigSection m) {
		if (m == null)
			throw new NullPointerException();
		this.section = m;
		this.name = loadName().toLowerCase();
	...
	private HashSet<String> loadWorlds() {
		...
		List<String> tempList = section.getStringList(PATH_WORLDS_LIST);

		if (tempList == null || tempList.isEmpty()) {
			if (shouldWorldsAutogen() == true && !section.isList(PATH_WORLDS_LIST))
			...
			if (tempList == null || tempList.isEmpty()) {
				...
				section.set(PATH_WORLDS_LIST, values);
				dirty = true;
			}
		} else {
			if (tempList != null)
	...
	private boolean loadUseWorldsAsBlacklist() {
		if (worlds.isEmpty())
			return true;
		if (!section.isBoolean(PATH_WORLDS_IS_BLACKLIST)) {
			boolean v = getUseWorldsAsBlacklistDefault();
			section.set(PATH_WORLDS_IS_BLACKLIST, v);
			dirty = true;
			return v;
		}
		return section.getBoolean(PATH_WORLDS_IS_BLACKLIST, true);
	}
	...
	private String loadDisplayName() {
		String tempDisplayName = section.getString(PATH_DISPLAY_NAME);
		if (tempDisplayName == null) {
			tempDisplayName = name.replace("_", " ");

			if (shouldAutogenDisplayName()) {
				section.set(PATH_DISPLAY_NAME, tempDisplayName);
				dirty = true;

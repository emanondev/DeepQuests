...
public abstract class YmlLoadableWithCooldown extends YmlLoadable {
	...
	public YmlLoadableWithCooldown(ConfigSection m) {
		super(m);
		repeatable = loadCooldownAllowed();
		minutes = Math.max(0, loadCooldownMinutes());
		this.addToEditor(8, new CooldownEditorFactory());
		this.addToEditor(5, new DisplayEditorFactory());
	}

	private boolean loadCooldownAllowed() {
		if (getSection() == null)
			throw new NullPointerException();
		if (!getSection().isBoolean(PATH_COOLDOWN_IS_ENABLED) && shouldCooldownAutogen()) {
			getSection().set(PATH_COOLDOWN_IS_ENABLED, getDefaultCooldownUse());
			dirty = true;
		}
		return getSection().getBoolean(PATH_COOLDOWN_IS_ENABLED, getDefaultCooldownUse());
	}

	private long loadCooldownMinutes() {
		if (getSection() == null)
			throw new NullPointerException();
		return getSection().getLong(PATH_COOLDOWN_AMOUNT, getDefaultCooldownMinutes());
	}
	...
	private class DisplayEditorFactory implements EditorButtonFactory {

		private class DisplayEditorButton extends CustomButton {
			private final ItemStack item = new ItemStack(Material.PAPER);

			public DisplayEditorButton(CustomGui parent) {
				super(parent);
				ItemMeta meta = item.getItemMeta();
				meta.setDisplayName(StringUtils.fixColorsAndHolders("&6&lFull Display Editor"));
			...
			private class DisplaySelector extends CustomLinkedGui<CustomButton> {

				public DisplaySelector(Player p) {
					super(p, DisplayEditorButton.this.getParent(), 6);
					this.setTitle(null, ChatColor.translateAlternateColorCodes('&', "&6Status Display Editor"));
					for(int i = 0; i < DisplayState.values().length ; i++) {
						this.addButton(i,new DisplayButton(this,DisplayState.values()[i]));
						this.addButton(i+9,new StateLoreEditorButtonFactory(DisplayState.values()[i]).getCustomButton(this));
						this.addButton(i+18,new ItemEditorButt(this,DisplayState.values()[i]));
						this.addButton(i+27,new HiddenFlagButton(this,DisplayState.values()[i]));
					}
					update();
				}
				private class DisplayButton extends CustomButton {
					public DisplayButton(CustomGui parent,DisplayState state) {
						super(parent);
						StringUtils.setDescription(item, 
								new ArrayList<String>(Arrays.asList("&6&lStatus: &6"+state.toString(),
										"&6"+state.getDescription(),"&6Click below buttons to edit:"
										,"&6Display Text, Display Item, Hidden status")));
					}
					private final ItemStack item = new ItemStack(Material.THIN_GLASS);
				...
				private class HiddenFlagButton extends CustomButton {
					private final DisplayState state;
					public HiddenFlagButton(CustomGui parent,DisplayState state) {
						super(parent);
						this.state = state;
						update();
					}
					private final ItemStack item = new ItemStack(Material.STAINED_GLASS_PANE);
					...
					public void update() {
						if (getDisplayInfo().isHidden(state)) {
							item.setDurability((short) 15);
							StringUtils.setDescription(item, new ArrayList<String>(Arrays.asList("&6This status is &chidden")));
						}
						else {
							item.setDurability((short) 0);
							StringUtils.setDescription(item, new ArrayList<String>(Arrays.asList("&6This status is &anot hidden")));
						}
					}

					@Override
					public void onClick(Player clicker, ClickType click) {
						getDisplayInfo().setHide(state, !getDisplayInfo().isHidden(state));
						update();
						getParent().update();
					}
				}
				private class StateLoreEditorButtonFactory extends StringListEditorButtonFactory {
					private final DisplayState state;
					public StateLoreEditorButtonFactory(DisplayState state) {
						super(new ArrayList<String>(), "DisplayTextEditor for "+state.toString(), Material.PAPER);
						this.state = state;
					}
					...
					protected void onChange(ArrayList<String> newList) {
						getDisplayInfo().setDescription(state, newList);
					}
					
				}
				
				private class ItemEditorButt extends ItemEditorButton {
					private final DisplayState state;
					public ItemEditorButt(CustomGui parent,DisplayState state) {
						super(parent);
						this.state = state;
					}
					@Override
					...
					@Override
					public void onClick(Player clicker, ClickType click) {
						requestItem(clicker,changeItemDesc);
					}
				}
	...
	private class CooldownEditorFactory implements EditorButtonFactory {
		private class CooldownEditorButton extends CustomButton {
			...
			@Override
			public void onClick(Player clicker, ClickType click) {
				clicker.openInventory(new CooldownEditorGui(clicker, (EditorGui) this.getParent()).getInventory());
			}
